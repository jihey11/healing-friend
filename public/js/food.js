...
export class FoodInventory {
  ...
  async save() {
    try {
      ...
      console.log('인벤토리 저장 시도:', { uid: this.uid, storageKey, items: this.items, dataLength: dataToSave.length });
      ...
      if (saved !== dataToSave) {
        console.error('인벤토리 저장 검증 실패! 저장된 데이터가 일치하지 않습니다.');
        return false;
      }
      ...
      if (error.name === 'QuotaExceededError') {
        console.error('localStorage 용량 초과! 일부 데이터를 삭제해야 합니다.');
      }
  ...
  async addFood(foodId, quantity = 1) {
    try {
      console.log('addFood 호출:', { foodId, quantity, uid: this.uid, currentItems: this.items });
      ...
      // uid 확인
      if (!this.uid) {
        console.error('addFood: uid가 없습니다!');
        return false;
      }
      
      // 기존 수량에 추가
      const previousQuantity = this.items[foodId] || 0;
      this.items[foodId] = previousQuantity + quantity;
      
      console.log(`음식 수량 업데이트: ${food.name} ${previousQuantity} -> ${this.items[foodId]}`);
      
      // 저장
      const saveResult = await this.save();
      console.log('저장 결과:', saveResult);
      console.log('저장 후 인벤토리:', this.items);
      ...
      console.log(`음식 추가 완료: ${food.name} x${quantity} (총 ${this.items[foodId]}개)`);
      return true;
 ...
 */
export async function feedCharacter(character, foodId, inventory, uid = null) {
  try {
    ...
    // 음식 등급 정보 가져오기 (가장 오래된 것부터 사용)
    let emotionPoints = 1; // 기본값
    let grade = 'D';
    ...
    if (character && character.updateEmotions) {
      ...
      console.log(`${food.emotion} 감정 점수 +${emotionPoints} (${grade}급)`);
    }
