// Firebase import (Firestore 데이터 저장용)
import { db } from './config.js';

// Firebase 함수를 동적으로 가져오는 헬퍼
    ...
    try {
      ...
      if (!this.uid.startsWith('demo_')) {
        ...
        if (firebaseFirestore && db && window.firebaseModules) {
          ...
          // Firestore에 저장 (merge: true로 기존 데이터 유지)
          await setDoc(userRef, characterData, { merge: true });
          console.log('✅ Firestore에 캐릭터 데이터 저장:', characterData);
          return;
        } else {
          console.warn('⚠️ Firebase가 초기화되지 않았습니다. 데이터를 저장할 수 없습니다.');
        }
      }
      ...
      if (this.uid.startsWith('demo_')) {
        ...
        console.log('캐릭터 데이터를 로컬 스토리지에 저장했습니다:', fullData);
        return;
      }
