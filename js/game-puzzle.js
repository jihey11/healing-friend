...
class PuzzleGame {
  ...
  handleTouch(e) {
    try {
      if (!this.isRunning || !this.imageLoaded) {
        console.log('게임이 실행 중이 아니거나 이미지가 로드되지 않았습니다');
        return;
      }
      ...
      console.log('퍼즐 터치 좌표:', { x, y, touchX: touch.clientX, touchY: touch.clientY, rect });
  ...
  processTileClick(x, y) {
    try {
      ...
      console.error('타일 클릭 처리 실패:', error);
    }
  }
   ...
   */
    ...
    try {
      ...
      // 캐릭터 모션 트리거
      this.triggerGameCompleteMotion(elapsed);
   ...
   * 게임 완료 시 캐릭터 모션 트리거
   */
  triggerGameCompleteMotion(timeInSeconds) {
    try {
      if (window.character && window.character.setMotionState) {
        // 시간에 따른 모션 결정 (빠를수록 좋음)
        if (timeInSeconds <= 30) {
          window.character.setMotionState('excited', 3000); // 30초 이하 - 흥분
        } else if (timeInSeconds <= 60) {
          window.character.setMotionState('happy', 2500); // 1분 이하 - 기쁨
        } else if (timeInSeconds <= 120) {
          window.character.setMotionState('happy', 2000); // 2분 이하 - 약간 기쁨
        } else {
          window.character.setMotionState('sad', 1500); // 2분 초과 - 슬픔
        }
      }
    } catch (error) {
...
async function incrementGamePlays(uid, gameType) {
  try {
    ...
    progress.lastReset = new Date().toISOString();
    
    localStorage.setItem(storageKey, JSON.stringify(progress));
    console.log(`${gameType} 게임 플레이 횟수 증가:`, progress[field]);
