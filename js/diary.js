...
class DiaryManager {
  ...
  async analyzeByGPT(text) {
    try {
      ...
      if (!API_URL) {
        console.warn('백엔드 API URL이 설정되지 않았습니다.');
        return null;
      }
  ...
  async canWriteToday() {
    try {
      ...
      if (this.uid.startsWith('demo_')) {
        const lastDiaryTime = localStorage.getItem('lastDiaryTime');
        if (!lastDiaryTime) {
          return { can: true, nextTime: null };
        }
  ...
  async saveDiary(content, selectedEmotion) {
    try {
      ...
      if (this.uid.startsWith('demo_')) {
        ...
        // 마지막 일기 작성 시간 저장
        localStorage.setItem('lastDiaryTime', new Date().toISOString());
...
export function setupDiaryUI(uid, character) {
  try {
    ...
    function checkCanSave() {
      try {
        const length = diaryContent.value.length;
        const hasContent = selectedEmotion && length >= 50;
    ...
    saveBtn.addEventListener('click', async () => {
      try {
        ...
        if (content.length < 50) {
          diaryError.textContent = '최소 50자 이상 작성해주세요.';
          return;
        }
